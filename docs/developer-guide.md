...
- `GET /discovery/resources`, `GET /discovery/search` — Bazaar catalog.
  ...
  search architecture" in `docs/architecture.md`. Every returned resource
  carries a `status` field, `"provisional"` (cataloged at a validated
  request, not yet settled — see "Automatic cataloging" in
  `docs/architecture.md`) or `"confirmed"` (a real settlement happened).
